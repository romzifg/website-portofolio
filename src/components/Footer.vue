<template>
	<footer class="py-10 sm:py-12" style="border-top: 1px solid var(--color-border);">
		<div class="max-w-7xl mx-auto px-6 lg:px-8">
			...
			<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
				...
				<div>
					...
					<p class="text-xs" style="color: var(--color-text-subtle); font-family: var(--font-mono);">{{ title }}</p>
				</div>
				...
				<div class="flex items-center gap-2">
					<a
						...
						class="footer-icon w-8 h-8 rounded flex items-center justify-center border transition-colors"
						style="border-color: var(--color-border); color: var(--color-text-subtle);"
					>
						<svg v-if="social.icon === 'github'" class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
						...
						<svg v-else-if="social.icon === 'linkedin'" class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
							<path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
						</svg>
						<svg v-else-if="social.icon === 'instagram'" class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
						...
						<svg v-else-if="social.icon === 'threads'" class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24">
				...
				<div class="flex items-center gap-5">
					<p class="text-xs" style="color: var(--color-text-subtle); font-family: var(--font-mono);">
						© {{ currentYear }} {{ name }}
					</p>
					<button
						@click="scrollToTop"
						class="back-top text-xs flex items-center gap-1.5 transition-colors"
						style="color: var(--color-text-subtle); font-family: var(--font-mono);"
...
.back-top:hover { color: var(--color-accent) !important; }
</style>
