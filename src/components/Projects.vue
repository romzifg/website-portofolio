		...
		<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
			...
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6 md:gap-8">
				...
				>
					...
					<div class="relative h-40 sm:h-48 md:h-56 bg-linear-to-br from-primary-500/20 to-pink-500/20 overflow-hidden">
						<img
							v-if="project.image"
							:src="project.image"
						...
						<div v-else class="absolute inset-0 flex items-center justify-center">
							<div
								class="text-5xl sm:text-6xl md:text-8xl opacity-50 group-hover:scale-110 transition-transform duration-300"
								role="img"
						...
						<div class="absolute top-3 right-3">
							<span class="px-2.5 sm:px-3 md:px-4 py-1 sm:py-1.5 glass rounded-full text-xs sm:text-sm font-medium backdrop-blur-md">
								{{ project.category || "Project" }}
							</span>
						</div>

						<!-- Links Overlay - tap-friendly on mobile -->
						<div
							v-if="project.link || project.github"
							class="absolute inset-0 bg-dark-900/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-3 sm:gap-4"
						>
							<a
								v-if="project.link"
								:href="project.link"
								...
								class="px-4 sm:px-5 md:px-6 py-2 sm:py-2.5 md:py-3 bg-primary-500 hover:bg-primary-600 rounded-lg font-semibold text-xs sm:text-sm md:text-base transition-colors inline-flex items-center gap-1.5 sm:gap-2"
								@click.stop
							...
							<a
								...
								target="_blank"
								rel="noopener noreferrer"
								class="px-4 sm:px-5 md:px-6 py-2 sm:py-2.5 md:py-3 glass hover:bg-white/20 rounded-lg font-semibold text-xs sm:text-sm md:text-base transition-colors inline-flex items-center gap-1.5 sm:gap-2"
								@click.stop
